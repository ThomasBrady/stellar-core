...
TEST_CASE("LedgerKeyMeter tests")
{
    ...
    // TTL entries do not count towards the read quota.
    auto ttl_entry = LedgerTestUtils::generateValidLedgerEntryWithTypes({TTL});
    auto ttl_key = LedgerEntryKey(ttl_entry);
    UnorderedSet<LedgerKey> ttl_keys;
    ttl_keys.emplace(ttl_key);
    resources.readBytes = 0;
    resources.footprint.readOnly = {ttl_key};
    lkMeter.addTxn(resources);

    // We can load a TTL entry of any size regardless of remaining quota.
    REQUIRE(
        lkMeter.canLoad(ttl_key, std::numeric_limits<std::uint32_t>::max()));
    entrySize = xdr::xdr_size(ttl_entry);
    lkMeter.updateReadQuotasForKey(ttl_key, entrySize);
    REQUIRE(
        lkMeter.canLoad(ttl_key, std::numeric_limits<std::uint32_t>::max()));
}
